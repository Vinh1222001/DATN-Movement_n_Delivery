...
{
  pinMode(LINE_COLOR_TRACKER_PIN_S2, OUTPUT);
  pinMode(LINE_COLOR_TRACKER_PIN_S3, OUTPUT);
  pinMode(LINE_COLOR_TRACKER_PIN_OUT, INPUT);
}
LineColorTracker::~LineColorTracker() {}

int LineColorTracker::getRed()
{
  digitalWrite(LINE_COLOR_TRACKER_PIN_S2, LOW);
  digitalWrite(LINE_COLOR_TRACKER_PIN_S3, LOW);
  return pulseIn(LINE_COLOR_TRACKER_PIN_OUT, LOW);
}

int LineColorTracker::getGreen()
{
  digitalWrite(LINE_COLOR_TRACKER_PIN_S2, HIGH);
  digitalWrite(LINE_COLOR_TRACKER_PIN_S3, HIGH);
  return pulseIn(LINE_COLOR_TRACKER_PIN_OUT, LOW);
}
